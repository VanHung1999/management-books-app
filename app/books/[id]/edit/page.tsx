...
export default function EditBook() {
  ...
  const [localBookData, setLocalBookData] = useState<any>(null);
  const [newTotal, setNewTotal] = useState<number>(0);
  const { notification } = App.useApp();
  
  const { mutate: updateBook, isPending: isUpdating } = useUpdate();
  const { data: bookData, isLoading: isBookLoading } = useOne({
    resource: "books",
    id: id as string,
  });
  ...
  useEffect(() => {
    if (bookData?.data) {
      setLocalBookData(bookData.data);
      const total = bookData.data.status.available + bookData.data.status.loaned + bookData.data.status.disabled + bookData.data.status.renovated;
      setNewTotal(total);
      form.setFieldsValue({ description: bookData.data.description || '' });
    }
  }, [bookData, form]);

  const updateNewTotal = (available: number, loaned: number, disabled: number, renovated: number) => {
    setNewTotal(available + loaned + disabled + renovated);
  };
  ...
  const onFinish = (values: { description: string }) => {
    updateBook({
      ...
      onSuccess: () => {
        ...
        // Update local state
        setLocalBookData((prev: any) => prev ? { ...prev, description: values.description } : null);
      },
    });
  };

  const onStatusFinish = (values: { available: number; loaned: number; disabled: number; renovated: number }) => {
    const book = localBookData || bookData?.data;
    if (!book) return;

    // Validation: Check if total remains the same (num should not change)
    const calculatedTotal = values.available + values.loaned + values.disabled + values.renovated;
    if (calculatedTotal !== book.num) {
      notification.error({ 
        message: "Validation Error", 
        description: `Total must equal ${book.num} (current total: ${book.num}). New total: ${calculatedTotal}` 
      });
      return;
    }
    ...
    updateBook({
      ...
      onSuccess: () => {
        ...
        // Update local state
        setLocalBookData((prev: any) => prev ? { 
          ...prev, 
          status: {
            available: values.available,
            loaned: values.loaned,
  ...
  const currentTotal = book.status.available + book.status.loaned + book.status.disabled + book.status.renovated;
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '0 32px 32px 32px' }}>
        ...
        <Card style={{ borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', marginBottom: '24px', border: 'none' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
            ...
            <div>
              <Title level={3} style={{ margin: '0 0 8px 0', color: '#1f1f1f' }}>{book.name}</Title>
              <Text style={{ color: '#8c8c8c' }}>Author: {book.author} • Category: {book.category}</Text>
            </div>
          </div>
        </Card>
        ...
        >
          <div style={{ marginBottom: '20px' }}>
            <Text strong style={{ fontSize: '16px', color: '#1f1f1f' }}>
              Total Copies (num): {book.num} • Current Total: {currentTotal}
            </Text>
            {currentTotal !== book.num && (
              <div style={{ marginTop: '8px', padding: '8px 12px', backgroundColor: '#fff2f0', border: '1px solid #ffccc7', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <Text style={{ color: '#ff4d4f', fontSize: '14px' }}>
                  Warning: Current total ({currentTotal}) doesn't match total copies ({book.num})
                </Text>
              </div>
            )}
          </div>
          ...
          {!isEditingStatus ? (
            <div>
              <Row gutter={[16, 16]}>
                {renderStatusCard('available', book.status.available, 'Available')}
                {renderStatusCard('loaned', book.status.loaned, 'Loaned')}
                {renderStatusCard('disabled', book.status.disabled, 'Disabled')}
                {renderStatusCard('renovated', book.status.renovated, 'Renovated')}
              </Row>
            ...
            <Form
              ...
              initialValues={{
                ...
                loaned: book.status.loaned,
                disabled: book.status.disabled,
                renovated: book.status.renovated,
              }}
            >
              <Row gutter={[16, 16]}>
                {renderInputField('available', 'Available', book.num, (value) => {
                  const currentValues = statusForm.getFieldsValue();
                  updateNewTotal(value || 0, currentValues.loaned || 0, currentValues.disabled || 0, currentValues.renovated || 0);
                })}
                 <Col xs={12} sm={6}>
                ...
                {renderInputField('disabled', 'Disabled', book.num, (value) => {
                  const currentValues = statusForm.getFieldsValue();
                  updateNewTotal(currentValues.available || 0, currentValues.loaned || 0, value || 0, currentValues.renovated || 0);
                })}
                {renderInputField('renovated', 'Renovated', book.num, (value) => {
                  const currentValues = statusForm.getFieldsValue();
                  updateNewTotal(currentValues.available || 0, currentValues.loaned || 0, currentValues.disabled || 0, value || 0);
                })}
              </Row>
                ...
                <div style={{ padding: '16px', backgroundColor: '#f6ffed', borderRadius: '8px', border: '1px solid #b7eb8f', marginBottom: '20px' }}>
                 <Text style={{ color: '#52c41a', fontSize: '14px' }}>
                   ...
                   • Total must equal {book.num} copies (Current: {newTotal})<br/>
              ...
              <Form.Item style={{ marginBottom: '0', textAlign: 'center' }}>
                <Space size="middle">
                  <Button onClick={() => {
                    ...
                    const book = localBookData || bookData?.data;
                    if (book) {
                      const total = book.status.available + book.status.loaned + book.status.disabled + book.status.renovated;
                      setNewTotal(total);
                    }
                  }}>Cancel</Button>
        ...
        <Card 
          ...
          style={{ borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', marginBottom: '24px', border: 'none' }}
        >
          <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ description: book.description || '' }}>
            <Form.Item
            ...
            <Form.Item style={{ marginBottom: '0', textAlign: 'right' }}>
              <Space size="middle">
                <Link href={`/books/${id}`}><Button size="large">Cancel</Button></Link>
                <Button
                  type="primary"
                  htmlType="submit"
                  size="large"
